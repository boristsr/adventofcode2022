...
free_space = total_diskspace - root_size
target_space = 30000000
required_space = target_space - free_space
print(f'free space: {free_space}, required: {required_space}')
...
while len(dirs_to_process) > 0:
    ...
    if curr_dir.calculate_size() >= required_space:
        found_dirs.append(curr_dir)

min_space = found_dirs[0].calculate_size()

for dir in found_dirs:
    if dir.calculate_size() < min_space:
        min_space = dir.calculate_size()
